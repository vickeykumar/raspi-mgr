...
def fanManager(loop):
	global FAN_ON
	loop_interval = 2	#poll evry 30 sec for temperature
	cpuTemp = getCPUtemp()
	if cpuTemp>THRESHOLD_TEMP :
		if not FAN_ON: fan(ON)
		'''wait 60 sec for ev 5'C raise in temp for next polling'''
		loop_interval = 2*loop_interval*math.ceil((cpuTemp-THRESHOLD_TEMP)/5)
	else:
		if FAN_ON: fan(OFF)
	print("CPU Temp: %d loop_after:%s"%(cpuTemp, loop_interval))
